...
function MyOrdersErrorFallback({ resetErrorBoundary }: FallbackProps) {
  return (
      ...
      <main className="max-w-container-max mx-auto px-4 md:px-6 py-6 min-h-[calc(100vh-200px)]">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <section className="flex-grow">
            <div className="bg-white rounded-xl p-4 md:p-6 shadow-sm border border-gray-100 text-center py-12">
              <span className="material-symbols-outlined text-5xl text-gray-300 mb-4 inline-block">error_outline</span>
              <h2 className="text-lg font-semibold text-gray-900 mb-2">Đã có lỗi xảy ra</h2>
              <p className="text-sm text-gray-500 mb-6">Vui lòng tải lại trang để thử lại.</p>
              <button
                onClick={() => window.location.reload()}
                className="bg-[#9f224e] text-white px-6 py-2.5 text-sm font-semibold rounded-lg hover:bg-[#7d1b3d] transition-colors cursor-pointer border-0"
              >
                Tải lại trang
              </button>
            </div>
...
const MyOrders: React.FC = () => {
        ...
        <main className="max-w-container-max mx-auto px-4 md:px-6 py-6 min-h-[calc(100vh-200px)]">
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <section className="flex-grow text-center py-12 max-w-md mx-auto">
              <span className="material-symbols-outlined text-5xl text-gray-300 mb-4 inline-block">login</span>
              <h2 className="text-lg font-semibold text-gray-900 mb-2">Chưa đăng nhập</h2>
              <p className="text-sm text-gray-500 mb-6">Vui lòng đăng nhập để xem lịch sử đơn hàng.</p>
              <Link to="/login" className="inline-flex items-center gap-2 bg-[#9f224e] text-white px-6 py-2.5 text-sm font-semibold rounded-lg hover:bg-[#7d1b3d] transition-colors no-underline">
                Đăng nhập ngay
              </Link>
            </section>
          </div>
  ...
  return (
    <ErrorBoundary FallbackComponent={MyOrdersErrorFallback}>
      <div className="bg-gray-50 min-h-screen pt-20">
                  ...
                  <>
                    <div className="md:hidden space-y-3">
                      {safeOrders.map((order: any, index: number) => {
                        ...
                        const subtotal = order?.orderDetails?.reduce((sum: number, item: any) => sum + (item?.unitPrice ?? 0) * (item?.quantity ?? 0), 0) ?? 0;
                        const displayTotal = (order?.finalAmount ?? 0) > 0 ? (order?.finalAmount ?? 0) : subtotal;
                        const statusText = order?.status;
                        const statusClass = statusText ? (statusStyles[statusText] || statusStyles.Pending) : statusStyles.Pending;
                        const totalQty = order?.orderDetails?.reduce((sum: number, item: any) => sum + (item?.quantity ?? 0), 0) ?? 0;
                        const orderId = order?.id ?? order?.orderId ?? order?.order_id;
                        const orderDate = order?.orderDate ?? order?.order_date;
                        return (
                          <div key={orderId ?? index} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                            <div className="flex items-center justify-between mb-3">
                              <span className="text-sm font-semibold text-gray-900">Mã đơn: #{orderId}</span>
                              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold ${statusClass}`}>
                                {statusText ? getOrderStatusText(statusText) : ''}
                              </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                              ...
                              <div className="flex items-center gap-2">
                                {statusText === 'PendingPayment' && (
                                  <button
                                    onClick={() => handleRetryPayment(orderId!)}
                                    disabled={retryingId === orderId}
                                  ...
                                  >
                                    {retryingId === orderId ? 'Đang xử lý...' : 'Thanh toán lại'}
                                  </button>
                                )}
                                <Link
                      ...
                      <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {safeOrders.map((order: any, index: number) => {
                            ...
                            return (
                              <tr key={orderId ?? index} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="py-4 px-4 text-sm font-medium text-gray-900">#{orderId}</td>
                                ...
                                <td className="py-4 pl-4 text-right whitespace-nowrap">
                                  <Link to={`/my-orders/${orderId}`} className="text-sm text-[#9f224e] hover:underline font-medium no-underline">
                                    Chi tiết
                                  </Link>
                                  {statusText === 'PendingPayment' && (
                                    <>
                                  ...
                                  {(statusText === 'Pending' || statusText === 'PendingVerification' || statusText === 'Confirmed' || statusText === 'PendingPayment') && (
                                    <>
                                      <span className="text-gray-300 mx-2">|</span>
                                      <button
                                        onClick={() => setCancelTarget(orderId!)}
